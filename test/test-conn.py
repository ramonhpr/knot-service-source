...
def get_data(msg): # TODO: use schemas e datas
    _, _, sensor_id = struct.unpack(HEADER_FMT + 'B', msg)
    logging.info('get_data: sensor_id: %s', sensor_id)
    if sensor_id == 251:
        send_knot_msg_push_data(s, sensor_id, 23.5)
    elif sensor_id == 252:
        send_knot_msg_push_data(s, sensor_id, 113)
    elif sensor_id == 253:
        send_knot_msg_push_data(s, sensor_id, False)
    elif sensor_id == 254:
        send_knot_msg_push_data(s, sensor_id, to_string('Test get_data'))

def set_data(msg): # TODO: use schemas e datas
