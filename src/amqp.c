...
struct amqp_context {
	...
	struct l_io *amqp_io;
	struct l_timeout *timeout_retry;
...
static void on_disconnect(struct l_io *io, void *user_data)
{
	...
	l_timeout_modify_ms(amqp_ctx.timeout_retry,
...
{
	amqp_ctx.timeout_retry = l_timeout_create_ms(1, // start in oneshot
				start_connection,
				settings, NULL);
...
void amqp_stop(void)
{
	...
	l_timeout_remove(amqp_ctx.timeout_retry);
